        ...
        async init() {
          ...
          for (let index = 0; index < mapInfo.length; index++) {
            ...
            for (let j = 0; j < colnum.length; j++) {
              ...
              const imgSrc = `tile (${item.src})`; // console.log('imgSrc', imgSrc)
              // sprite.spriteFrame = spriteFrames.find(v => v.name === imgSrc) || spriteFrames[0]

              console.log(spriteFrames[1].name);
              spriteFrames.find(v => {// console.log(v.name)
              });
...
//# sourceMappingURL=1b15f913d4f13fb13d13f98da1df9b286c6aeafd.js.map